            ...
            # Input (random element from the dataset)
                    ...
                    if n_hidden_layer>0: 
                        ...
                        if n_hidden_layer2 > 0:
                            ...
                            validation_error[values][num_tries] = validation_error[values][num_tries] + 0.5*np.sum(np.square(e_n))/randomised_dev.shape[0] + lambdalist[values] * np.sum(np.absolute(W1))/randomised_dev.shape[0] + lambdalist[values] * np.sum(np.absolute(W2))/randomised_dev.shape[0] + lambdalist[values] * np.sum(np.absolute(W3))/randomised_dev.shape[0]
                        else:
                            # Compute the error signal
                            e_n = desired_output - x2
                            validation_error[values][num_tries] = validation_error[values][num_tries] + 0.5*np.sum(np.square(e_n))/randomised_dev.shape[0] + lambdalist[values] * np.sum(np.absolute(W1))/randomised_dev.shape[0] + lambdalist[values] * np.sum(np.absolute(W2))/randomised_dev.shape[0]
                    else:
                        # Compute the error signal
                        e_n = desired_output - x1
                        validation_error[values][num_tries] = validation_error[values][num_tries] + 0.5*np.sum(np.square(e_n))/randomised_dev.shape[0] + lambdalist[values] * np.sum(np.absolute(W1))/randomised_dev.shape[0]
...
plt.ylabel('Error')
plt.errorbar(lambdalist, mean_error, yerr=yerr)
